...
% Last modified: 11-Feb-2016
    ...
    properties (Constant)        
      %  dataDir = '../hw2data';
        dataDir = 'E:\AA_homework\cse512\CSE512_MachineLearning-master\HW2\hw2data';
    ...
    methods (Static)
        ...
        function rects = detect(im, w, b, shldDisplay)
            ...
            for s = 1:length(scales) % consider each scale in turn
                ...
                hogIm = vl_hog(im2single(gpuArray(scaleIm)), HW2_Utils.hogCellSz);
        ...
        function [D, lb, imRegs] = getPosAndRandomNegHelper(dataset)
            ...
            for i=1:length(ubAnno)
                ...
                fprintf('%s/%sIms/%04d.jpg\n', HW2_Utils.dataDir, dataset, i)
